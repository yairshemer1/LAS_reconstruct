...
class Trainer:
    ...
    def test(self):
        ...
        for x, y in tqdm(self.test_loader):
            ...
            transcript = get_words(result.detach().cpu().numpy(), self.tokenizer, need_argmax=True)
            gt_transcript = get_words(y.detach().cpu().numpy(), self.tokenizer, need_argmax=False)
            for gt, pred in zip(gt_transcript, transcript):
                wer_arr.append(wer(gt, pred))
                cer_arr.append(cer(gt, pred))
        ...
        if "wer" not in self.history:
            self.history['wer'] = [np.mean(wer_arr)]
        else:
            self.history['wer'].append(np.mean(wer_arr))
        if "cer" not in self.history:
            self.history['cer'] = [np.mean(cer_arr)]
        else:
            self.history['cer'].append(np.mean(cer_arr))
...
def get_words(labels, tokenizer, need_argmax=False) -> list:
    sen_lst = []
    if need_argmax:
        labels = np.argmax(labels, axis=2)
    for sentence in labels:
        chars = list(map(lambda x: tokenizer._id_to_token[x], sentence))
        trans = "".join(chars)
        sos_loc = trans.find("<SOS>")
        if sos_loc != -1:
            trans = trans[sos_loc + 5:]
        eos_loc = trans.find("<EOS>")
        if eos_loc != -1:
            trans = trans[:eos_loc]
        trans.replace("<PAD>", "")
        sen_lst.append(trans)
    return sen_lst
